...
void VM::initMetrics()
{
    static bool done = []() noexcept
    {
        ...
        std::memcpy(&c_metrics[uint8_t(Instruction::PUSHC)],
            &c_metrics[uint8_t(Instruction::PUSH1)], sizeof(evmc_instruction_metrics));
        std::memcpy(&c_metrics[uint8_t(Instruction::JUMPC)], &c_metrics[uint8_t(Instruction::JUMP)],
            sizeof(evmc_instruction_metrics));
        std::memcpy(&c_metrics[uint8_t(Instruction::JUMPCI)],
            &c_metrics[uint8_t(Instruction::JUMPI)], sizeof(evmc_instruction_metrics));
